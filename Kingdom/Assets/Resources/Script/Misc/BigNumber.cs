...
public struct BigNumber
{
    ...
    public static BigNumber operator <<(BigNumber a, int b) => a * Pow(2, b);
    public static BigNumber operator >>(BigNumber a, int b) => Floor(a / Pow(2, b));
    ...
    public static BigNumber Cube(BigNumber num) => Pow(num, 3.0);
    public static BigNumber Square(BigNumber num) => Pow(Abs(num), 2.0);
    ...
    public static BigNumber Log(BigNumber num, double newBase)
    {
        if (num.Sign <= 0) throw new ArgumentException("Logarithm of non-positive number");
        return new BigNumber(num.Power * Math.Log(newBase) / Math.Log(10), 1);
    }
    public static BigNumber Clamp(BigNumber num, BigNumber min, BigNumber max)
    {
        if (num >= max) return max;
        if (num <= min) return min;
        return num;
    }
    public static BigNumber Clamp01(BigNumber num) => Clamp(num, 0, 1);
    public static BigNumber Repeat(BigNumber t, BigNumber length) => Clamp(t - Floor(t / length) * length, 0f, length);
    public static BigNumber Lerp(BigNumber a, BigNumber b, BigNumber t) => a + (b - a) * t;
    ...
    public static BigNumber Ceiling(BigNumber num)
    {
        if (num.Sign == 0) return 0;
        if (num.Sign < 0) return -Floor(-num);

        if (num.Power < 0) return 1;
        if (num.Power > 8) return num;
        double value = Math.Pow(10, num.Power) * num.Sign;
        double ceiledValue = Math.Ceiling(value);

        if (ceiledValue == 0) return 0;
        return new BigNumber(Math.Log10(Math.Abs(ceiledValue)), Math.Sign(ceiledValue));
    }
    public static BigNumber Floor(BigNumber num)
    {
        if (num.Sign == 0) return 0;
        if (num.Sign < 0) return -Ceiling(-num);

        if (num.Power < 0) return 0;
        if (num.Power > 8) return num;
        double value = Math.Pow(10, num.Power) * num.Sign;
        double flooredValue = Math.Floor(value);

        if (flooredValue == 0) return 0;
        return new BigNumber(Math.Log10(Math.Abs(flooredValue)), Math.Sign(flooredValue));
    }
